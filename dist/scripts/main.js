(function () {
    ...
    (function (window, undefined) {
            ...
            onTouch: function onTouch(element, eventType, handler) {
                ...
                var onTouchHandler = function onTouchHandler(ev) {
                    var src_type = ev.type.toLowerCase(),
                        ...
                        trigger_type, is_mouse = Utils.inStr(src_type, 'mouse');
        ...
        // AMD export
        if (typeof define == 'function' && define.amd) {
            define('plugins/hammer', [], function () {
                return Hammer;
            });
        }
        // commonjs export
        ...
        $(function () {
            ...
            (function ($, window, document, undefined) {
                ...
                $.baseline = function (options) {
                    ...
                    var el = !!options.el ? options.el : 'p';
                    var exec = {
                        getMult: function () {
                            var para = $('body').find(el).first(),
                                lh = parseInt(para.css('line-height'), 10);
                            return lh;
                        },
                        writeBaseline: function () {
                            var docHeight = $(document).height(),
                                mult = this.getMult(),
                                count = Math.round(docHeight / mult),
                                newTop = mult;

                            for (var i = count - 1; i >= 0; i--) {
                                $('body').append($('<div class="baseline" style="top:' + newTop + 'px"/>'));
                                newTop = newTop + mult;
                            };
                        }
                    };
            ...
            ').appendTo($('head'));
                    ...
                    $(document).on('keyup', function (e) {
                        ...
                        if (e.ctrlKey && e.which === 71) { // CTRL + G

                            var $baseline = $('.baseline');

                            if (!$baseline.length) {
                                exec.writeBaseline();
                            } else if (winWidth === $(window).width() && $baseline.length) {
                                $baseline.toggleClass('invisible');
                            } else if (winWidth !== $(window).width() && $baseline.length) {
                                winWidth = $(window).width();
                                var wasVisible = !!!$baseline.hasClass('invisible');
                                $baseline.remove();
                                exec.writeBaseline();
                                if (wasVisible) {
                                    $('.baseline').toggleClass('invisible');
